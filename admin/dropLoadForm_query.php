...
include '../config.php';
    if (isset($_POST['submit'])) {
                ...
                $sql_val = "SELECT * FROM loads WHERE loads.id = '".$load_id."' ";
                $query_val = mysqli_query($conn, $sql_val) or die(mysqli_error($conn));
                $row_val = mysqli_fetch_assoc($query_val);
                
                if($row_val["totalLoadPicked"] < 1){
                    echo '<script> alert("No load Picked, Ensure the load is Picked before Delivery."); window.location.href = "assigned_load.php";</script>';
                }
                else{
          ...
          if ($query) {
             echo '<script> alert("Load Successfully Delivered"); window.location.href = "assigned_load.php";</script>';
