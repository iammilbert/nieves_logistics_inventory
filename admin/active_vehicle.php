          ...
          <div class="col">
            <div class="card card-primary card-outline">
              <div class="card-header">
                <h3 class="card-title"><b style="color:red;">ACTIVE VEHICLES</b></h3>
              </div>
              <div class="card-body">
                ...
                <table id="example1" class="table" style="font-size:13.5px;">
                  ...
                  <tbody>
                   ...
                   <?php
                      $date = date("d-m-Y");
                      $sql = "SELECT vehicles.vin, vehicles.model, vehicles.number, vehicles.vehicleType, vehicles.id, users.name, loads_assigned.status, vehicles_assigned.driver_id, loads_assigned.vehicle_id FROM vehicles_assigned
                      INNER JOIN vehicles
                      ON vehicles_assigned.vehicle_id = vehicles.id
                      INNER JOIN users
                      ON vehicles_assigned.driver_id = users.id 
                       INNER JOIN loads_assigned
                      ON loads_assigned.vehicle_id = vehicles.id 
                      WHERE loads_assigned.status = 2 AND vehicles_assigned.date_assigned = '".$date."'";
                    ...
                    ?>
                      ...
                      <tr>
                        ...
                        <?php 
                      if($row["status"] == 1)
                        echo "<small class='badge badge-primary'>Assigned, but not Active</small>";
                         if($row["status"] == 2)
                        echo "<small class='badge badge-info'>On Trip (Active)</small>";
                        elseif($row["status"] == 3)  
                         echo "<small class='badge badge-success'>Load Delivered</small> ";
                        elseif($row["status"] == 4) 
                      ...
                      ?></td>
                        ...
                        <td>
                          <a title="Delete vehicle" name="delete" class="btn btn-sm btn-danger deletebtn"><i class="fa fa-trash"></i></a>

                          <a class="btn btn-sm btn-info" data-toggle="modal" data-target="#vehicle_details<?php echo $row['id']?>"><i class="fa fa-eye"></i></a>
        ...
        <div class="modal-dialog">
          <div class="modal-content">
            <div class="modal-header bg-primary">
              <h4 class="modal-title">Truck Registration</h4>
            ...
            <div class="car card-primary">
                         <!-- form start -->
              <form class="form" action="signupCheck.php" method="POST">
                <div class="card-body">
                  <div class="form-group">
                    <label for="truckNumber">Truck Number</label>
                    <input type="text" name="truckNumber" class="form-control" id="truckNumber" placeholder="Enter Truck Number">
                  </div>

                  <div class="form-group">
                    <label for="truckVin">VIN</label>
                    <input type="text" name="truckVin" class="form-control" id="truckVin" placeholder="Enter VIN">
                  </div>

                  <div class="form-group">
                    <label for="truckModel">Truck Model</label>
                    <input type="text" name="truckModel" class="form-control" id="truckModel" placeholder="Enter Truck Model">
                  </div>
                </div>
                <!-- /.card-body -->
              <div class="modal-footer justify-content-between bg-primary">
                  <button type="submit" class="btn btn-outline-light btn-primary">Submit</button>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="exampleModalLabel"> Delete Truck Data </h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
                ...
                <form action="delete_truck_query.php" method="POST">
                   <div class="modal-body">
                        ...
                        <h4> Do you want to Delete this Truck ??</h4>
                    </div>
