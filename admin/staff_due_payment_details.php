               ...
               <table id="example1" class="table table-striped" style="font-size:14px;">
                  ...
                  <tr>
                    ...
                    <td><b><?php $months = new DateTime($row["months"]); echo $months->format('M-Y'); ?></b></td>
                      ...
                      <td><small class="badge badge-info"><?php if($row["status"]==1) { echo "Paid";} else echo "Pending"; ?></small></td>
                      
                        
                     <td>  
                     <a class="btn btn-primary" href="editpayRollForm.php?staffId=<?php echo $row["staff_id"]; ?>"><i class="fa fa-edit"></i></a>
                     
                      <?php 
                         if($row["status"]==1)
                        echo "<a class='btn btn-sm btn-info' href='payRollSlip.php?paidSlipID={$row["id"]}'><i class='fa fa-eye'></i> Slip</a>";
                        
                        elseif ($row["status"] == 0) {
                          echo "<a class='btn btn-sm btn-success' href='payRollSlip.php?paidSlipID={$row["id"]}'><i class='fa fa-eye'></i> Make Payement</a>  ";
