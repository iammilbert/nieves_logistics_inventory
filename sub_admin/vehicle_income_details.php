              ...
              <div class="card-body">
                <table id="example1" class="table font-size-13">
                  ...
                  if (isset($_POST["generate"])) {
                      ...
                      $sql="SELECT loads.rateConfirmationID, loads.rate, loads.netPay, loads_assigned.status, loads_assigned.vehicle_id, loads_assigned.load_id FROM loads
                      INNER JOIN loads_assigned 
                      ON loads_assigned.load_id = loads.id 
                      WHERE loads_assigned.vehicle_id = '".$vehicleID."' AND loads_assigned.dropped_Date BETWEEN '".$startDate."' AND '".$endDate."'";
                    ...
                    <?php                      
                      ...
                      $sql="SELECT loads.rateConfirmationID, loads.rate, loads.netPay, loads_assigned.status, loads_assigned.vehicle_id, loads_assigned.load_id FROM loads
                      INNER JOIN loads_assigned 
                      ON loads_assigned.load_id = loads.id WHERE loads_assigned.vehicle_id = '".$vehicleID."' ";
                      $query = mysqli_query($conn, $sql) or die(mysqli_error($conn));
        ...
        <!-- New row -->
          ...
          <div class="col">
            <div class="card card-primary card-outline">
              ...
              <?php  
                  $vehicleID  = isset($_GET['vehicleID']) ? $_GET['vehicleID'] : ''; 
                  $sql2 = "SELECT SUM(loads.netPay) AS sum, SUM(loads.rate) AS sum2, loads_assigned.vehicle_id FROM loads 
                      INNER JOIN loads_assigned 
                      ON loads_assigned.load_id = loads.id WHERE loads_assigned.vehicle_id = '".$vehicleID."' ";
