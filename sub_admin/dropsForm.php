          ...
          <div class="col-md-8">
            <div class="card card-primary card-outline">
              <div class="card-header">
                <h3 class="card-title"><b style="color:red;">Drop Information</b></h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
              ...
              <form method="POST" action="drops_query.php">
                <div class="card-body"> 
                  ...
                  <div class="row">
                    ...
                    <div class="col-md-6">
                      <div class="input-group mb-3">
                          <label for="rateConfirmationID" class="input-group-text">Total Drops</label>
                            <?php 
                               $sql = "SELECT COUNT(drops.id) AS sum FROM drops WHERE drops.load_id = '".$id."'"; 
                               $query = mysqli_query($conn, $sql) or die(mysqli_error($conn));
                            ?>
                            <?php while ($id_row = mysqli_fetch_assoc($query)): ?> 
                          <input type="text" class="form-control" name="rateConfirmationID" readonly value="<?php echo $id_row["sum"]; ?>">
                           <?php endwhile ?>
                        </div>
                    </div>
                  </div>
                         ...
                         <div class="input-group mb-3">
                          <span for="name" class="input-group-text">Drop name</span>
                          <input type="text" class="form-control" name="name" id="name" placeholder="Enter Drop name" required="name">
                        </div>

                         <div class="input-group mb-3">
                          <span for="state" class="input-group-text">Delivery State</span>
                          <input type="text" class="form-control" name="state" id="state" placeholder="Enter Delivery State" required="state">
                        </div>

                       <div class="input-group mb-3">
                          <span for="city" class="input-group-text">City</span>
                          <input type="location" class="form-control" name="city" id="city" placeholder="Enter Drop City" required="city">
                        </div>

                        <div class="input-group mb-3">
                          <span for="address" class="input-group-text">Drop Address</span>
                          <input type="location" class="form-control" name="address" id="address" placeholder="Enter Drop address" required="address">
                        </div>
                        ...
                        <div class="input-group mb-3">
                          <span for="date" class="input-group-text">Drop Date</span>
                          <input type="date" class="form-control" name="date" id="date" placeholder="Enter Drop date" required="date">
                          <input type="hidden" class="form-control" name="status" id="status" value="1">
                          <input type="hidden" class="form-control" name="load_id" id="status" value="<?php echo $row["id"]; ?>">
                        </div>

                        <div class="input-group mb-3">
                          <span for="time" class="input-group-text">Drop Time</span>
                          <input type="time" class="form-control" name="time" id="time" placeholder="Enter Drop time" required="time">
                           <input type="hidden" class="form-control" name="totalDrops">
                        </div>
                </div>
                <!-- /.card-body -->
                <div style="clear:both;"></div>
              <div class="modal-footer justify-content-between">
               <a href="dropsTable.php?reg=<?php echo $row["id"]; ?>" class="btn btn-danger">Back</a>
