...
<?php 
   if (isset($_POST['view'])) { 
    ...
    $id = $_POST['id']; 
          ...
          $sql_sum = "SELECT loads.driver_id, vehicles_assigned.vehicle_id, vehicles_assigned.date_assigned, SUM(drivers_pay_roll.netPay) AS sum, users.name, SUM(loads.rate) AS sum2, COUNT(loads.id) AS count,  loads_assigned.assignedBy, loads.dropped_Date, loads.rateConfirmationID FROM loads 
            ...
            WHERE vehicles_assigned.vehicle_id = '".$id."' AND loads.paymentStatus = 1  AND loads.dropped_Date  BETWEEN '".$startDate."' AND '".$endDate."'";
...
<?php  
          ...
          $sql_sum_grand = "SELECT loads.driver_id, vehicles_assigned.vehicle_id, vehicles_assigned.date_assigned, SUM(drivers_pay_roll.netPay) AS sum, SUM(loads.rate) AS sum2, users.name, loads_assigned.assignedBy, loads.rateConfirmationID FROM loads 
            ...
            WHERE vehicles_assigned.vehicle_id = '".$id."' AND loads.paymentStatus = 1";
            ...
            <div class="col"> <!-- New col --> 
              <div class="card card"> <!-- New card --> 
                 <div class="card-header">
                  <h3 class="card-title"><b>VEHICLE REPORT FROM</b>  <small class="badge badge-warning"><?php $start = $_POST['startDate']; echo DateTime::createFromFormat('Y-m-d', $start)->format('l jS F Y');?></small> - <small class="badge badge-success"><?php $end = $_POST['endDate']; echo DateTime::createFromFormat('Y-m-d', $end)->format('l jS F Y'); ?></small></h3>
                </div>
                <div class="card-body"><!-- New body --> 
                  ...
                  <tbody>
                    ...
                    $sql = "SELECT loads.driver_id, vehicles_assigned.vehicle_id, vehicles_assigned.date_assigned, drivers_pay_roll.netPay, users.name, loads.rate, loads.bol, loads.bolStatus, loads.id, loads_assigned.assignedBy, loads.dropped_Date, loads.rateConfirmationID FROM loads 
                    INNER JOIN vehicles_assigned
                    ON vehicles_assigned.load_id = loads.id 
                  ...
                  <tr>
                    ...
                    echo $earn; ?> <sup class="bold text-green">USD</sup>
                ...
                <tfoot>
                  <tr>
                    ...
                    <th></th>
                  <th>SUM TOTAL</th> 
                  <th><h3 class="badge badge-success">$<?php $earning = $id_row["sum2"] - $id_row["sum"]; echo number_format($earning, 2, '.', ',');?><sup>USD</sup></h3></th>
                  </tr>
                </tfoot>
                </table>                   
              </div><!-- /.card-body -->
