...
include '../config.php';
    if (isset($_POST['submit'])) {
              ...
              $sql = "INSERT INTO drivers_pay_roll (load_id, driver_expenses, driver_id, deduction, netPay, deduction_reason, medicalAllowance, dataAllowance, tax, loan, percent, basicPayAmount) VALUES('".$load_id."', '".$driver_expenses."', '".$driver_id."', '".$deduction."', '".$netPay."', '".$deduction_reason."', '".$medicalAllowance."', '".$dataAllowance."', '".$tax."', '".$loan."', '".$percent."', '".$basicPayAmount."')"; 
     ...
     if ($query) {
             echo '<script> alert("Pay Roll Registered"); window.location.href = "load_delivered.php";</script>';
             
           }else { 
                echo '<script> alert("Unknown Error Occur, register again"); window.location.href = "load_delivered.php";</script>';
